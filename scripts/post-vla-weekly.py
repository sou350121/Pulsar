...
# -*- coding: utf-8 -*-
"""
VLA Weekly Deep Dive - Phase 3: Post-processor.

- Merge LLM output into memory/vla-weekly-digest.json (idempotent per-date upsert)
- Optionally send Telegram update using payload.telegram_text (best-effort)

GitHub push of full weekly article is handled separately by the Theory pipeline;
this script only handles memory write + Telegram notification.
...
WEEKLY_PATH = os.path.join(MEM_DIR, "vla-weekly-digest.json")
...
def _sanitize_entry(day, payload):
    ...
    out["date"] = day
    for k in ("tldr", "spotlight", "sota_highlights", "release_highlights",
               "social_highlights", "industry_moves", "next_week_outlook"):
        v = out.get(k)
        if v is None:
            out[k] = []
...
def _upsert_weekly(day, entry, dry_run=False):
    obj = _read_json(WEEKLY_PATH, {"vla_weekly_digest": []})
    rows = obj.get("vla_weekly_digest")
    ...
    out_obj = {"vla_weekly_digest": rows}
    if not dry_run:
...
def main():
    ...
    try:
        ...
        print(json.dumps({"ok": False, "date": day, "error": "weekly_write_failed",
                          "detail": str(e)[:220]}, ensure_ascii=False))
        return 1
