...
- **Watchdog 自愈系统 → 管道故障自恢复**：16 项健康监控，7 类故障场景自动重跑，日志全程记录
...
```bash
python3 scripts/check-pipeline.py            # 完整检查（~30 秒）
python3 scripts/check-pipeline.py --parse    # 只跑 AST 解析，快
python3 scripts/check-pipeline.py --quiet    # 隐藏逐文件 OK 行
```

三件事：AST 解析所有 `scripts/*.py`、确认共享 helper（`_vla_expert.py` /
`_domain_loader.py` / `_gh_issues_config.py`）能导入、对叶子机械脚本（场态触
发器、跨域引擎、GH 采纳分析、社区上下文打包、GH issue 采集器）在临时空
memory 目录下做 smoke run。Day-1「无上游数据」的退出码会和预期错误消息严
格匹配，所以**在你跑第一个真实 cron 之前**这个检查就有意义。
...
`scripts/daily-watchdog.py` 每天运行，检查 16 项健康指标：
...
| 规则 ID | 方向 | 触发条件 |
|---------|------|---------|
| **R001** | VLA 技术 → AI App | Diffusion / flow matching / Transformer / RLHF / 量化等技术外溢 |
| **R002** | AI App 框架 → VLA | 新 agent 框架（LangGraph、CrewAI 等）被机器人栈采纳 |
| **R003** | AI 具身 → VLA | 通用实验室的具身 AI 论文进入 VLA 视野 |
| **R004** | VLA 基础 → AI | 起源于机器人领域的基础模型 / 预训练方法回流 AI |
| **R005** | 范式融合 | 同一周两个领域汇聚到同一范式 |
| **R006** | GitHub 仓库收敛 | top issues / PR 同时触及共享依赖（PyTorch、JAX、Triton） |
| **R007** | 假设驱动迁移 | LLM 主动生成的迁移假设（AI → VLA），每周期限额 |

输出：`memory/cross-domain-insight.json`；报告会引用最新 insight 与对应的 LLM 意义说明。
...
```
collect-github-issues.py  （每日，所有 tier-1 仓库）
       ↓
  memory/gh-issues-*.json
       ↓
compute-gh-adoption.py    （周五 13:00 —— tier-1 + tier-2）
       ↓
  采纳阶段 · DFI（Daily Field Index）· 收敛信号
       ↓
update-gh-field-notes.py  （推回你的知识库）
```

- **仓库注册表**：`scripts/_gh_issues_config.py` —— 改成你领域的仓库即可（tier-1 = 每日，tier-2 = 每周）
- **采纳阶段**：孵化 → 成长 → 主流 → 成熟，依据 issue 节奏 + 社区提问密度
- **收敛信号**：≥3 个被监控仓库在同一周指向同一依赖 / 方法 / Benchmark
- **环境变量**：`PULSAR_FIELD_NOTES_REPO` / `PULSAR_FIELD_NOTES_PATH` —— 指向你自己的知识库文件
...
| 触发器 | 捕捉的现象 |
|--------|----------|
| `breakthrough_density` | 3 天窗口内 ⚡ 级信号扎堆 |
| `paradigm_shift` | 新方法族进入 top-5 份额 |
| `consensus_drift` | 某条假设被反复证伪 |
| `silent_decay` | 曾经热门的方法族连续 14+ 天消失 |
| `cross_domain_pull` | 外域技术穿越进入评分信号 |
| `release_clustering` | 7 天内 ≥3 次模型发布瞄准同一 benchmark |

场态触发器跑在 deep-dive 排程**之前**——只有命中的信号才进入 deep-dive 候选池。机械筛选把 LLM 成本钉死，输出可审计。

---

### 语义记忆搜索

Pulsar 维护一个滚动 60 天窗口的嵌入索引：

```bash
# 增量构建 / 刷新（batch=10）
python3 scripts/semantic-index-builder.py

# 纯 Python 余弦查询，零 torch / numpy 依赖
python3 scripts/semantic-search.py "上个月哪些证据反驳了假设 V-003？"
```

- **嵌入模型**：DashScope `text-embedding-v3`（1024 维）；任意 OpenAI 兼容的 embedding 端点都能换，一行 URL 改动
- **存储**：`memory/semantic-index/chunks.jsonl` + `memory/semantic-index/vectors.bin`
- **检索**：chunk 向量 top-k 余弦相似度，返回 `{source, date, snippet, score}`
- **MCP 接入**：同时通过 `search_memory` MCP 工具暴露给 Claude Desktop

---

### 多领域路由

`memory/domains.json` 注册表 + `scripts/_domain_loader.py` 共享加载器——加一个新领域只需要改一个文件。每个领域有独立的 `active-config.json`、假设和 Telegram 路由配置。
...
│   ├── daily-watchdog.py       # 健康监控 + 自愈（16 项检查）
│   ├── cross-domain-rule-engine.py  # 7 条内建规则（R001-R007）跨域桥接
│   ├── ai-field-state.py       # 机械化场态触发器（无 LLM，6 类触发器）
│   ├── collect-github-issues.py     # 每日 OSS 仓库 issue 收集
│   ├── compute-gh-adoption.py       # 每周采纳阶段分析（DFI / 收敛信号）
│   ├── update-gh-field-notes.py     # 把 field-notes 推回你的知识库
│   ├── prep-community-context.py    # 给周报/双周报打包社区 + 采纳上下文
│   ├── semantic-index-builder.py    # DashScope text-embedding-v3，增量构建
│   ├── semantic-search.py      # 纯 Python 余弦相似度查询
│   ├── entity-tracker.py       # 90 天窗口的作者/实验室/方法/Benchmark 索引
│   ├── upstream-signal-monitor.py   # 跟踪 1-2 个上游领域捕捉早期信号
...
| **免疫系统** | 自身免疫 | Watchdog：16 项健康检查，7 条自愈路径 |
...
| Pipeline 脚本 | **67** 个，覆盖 VLA / AI / 校准 / 跨域 / GH 采纳 / 语义记忆 多条 pipeline |
| 追踪假设 | **19** 条，月度置信度自动更新 |
| Watchdog 检查 | **16** 项健康指标，**7** 条自愈路径 |
| 跨域规则 | **7** 条内建规则（R001-R007）—— VLA 技术迁移、AI 框架采纳、范式融合、GitHub 仓库收敛、假设驱动迁移 |
| MCP 工具 | **12** 个只读工具（信号、知识、元数据、搜索、领域注册表） |
...
| 功能 | 说明 |
|------|------|
| **跨域规则引擎 v2** | `cross-domain-rule-engine.py` 升级到 7 条内建规则（R001-R007）—— 覆盖 VLA→AI 技术迁移、AI→VLA 框架采纳、具身 AI 桥接、基础模型回流、范式融合、GitHub 仓库收敛、LLM 假设驱动迁移。每批 insight 现在附带一句 LLM「跨域意义」总结。 |
| **GitHub Issues 采纳度传感器** | 新 4 脚本管线（`collect-github-issues.py`、`compute-gh-adoption.py`、`_gh_issues_config.py`、`update-gh-field-notes.py`）—— 监控 OSS 仓库注册表，推断采纳阶段（孵化→成长→主流→成熟）、Daily Field Index（DFI）与跨仓库收敛信号。可通过环境变量把输出推回你自己的知识库。 |
| **场态触发器** | 新 `ai-field-state.py` —— 机械化、零 LLM 的 6 类触发器闸门（突破密度、范式偏移、共识漂移、静默衰减、跨域拉力、发布扎堆）。在 deep-dive 排程**之前**跑，把 LLM 成本钉死。 |
| **社区上下文打包** | 新 `prep-community-context.py` —— 从你的知识库取社区 field notes 紧凑摘要 + 最新采纳快照，打包成一份 tmp 文件供周报 / 双周报读。目标 repo 可通过 `PULSAR_FIELD_NOTES_REPO` 覆盖。 |
| **MCP Server：12 工具** | 文档与计数已对齐：`list_domains`、`get_domain_config`、`search_memory` 与原始 9 个工具一并文档化。先前 11 的计数为错。 |
| **Watchdog：16 项检查** | 计数从 15 修正到 16（Drift Detector 与 AI deep-dive 上线后新增 `quality_drift`、`ai_deep_dive` 跟随检查）。 |
| **架构文档** | 新 `docs/architecture.md` 完整呈现 4 层模型（INPUT → PROCESSING → OUTPUT → QUALITY）与自我纠错闭环。 |
| **自检脚本** | 新 `scripts/check-pipeline.py` —— AST 解析所有脚本、确认共享 helper 可导入、对零 LLM 叶子脚本在空 memory 目录下做 smoke run；Day-1 预期退出码与错误消息严格匹配，所以**第一个 cron 跑之前**这个检查就有意义。 |
| **路线图行** | 场态触发器与 GitHub Issues 采纳度传感器加入路线图（标记 ✅ 已完成，先前仅出现在「进阶能力」一节）。 |
| **Bug 修复** | `prep-community-context.py` 有 3 处字串字面量壞掉（內嵌字面換行而非 `\n` 转义）—— import 即报 `SyntaxError`。同 bug 存在于维护者私有 fork；本 patch 只修 OSS 模板版本。 |
...
| **P1** | [**跨域规则引擎**](docs/use-cases/cross-domain-rule-engine.md) → [v2](docs/use-cases/cross-domain-engine-v2.md) | 用户定义确定性规则实现跨域信号桥接：`IF vla_rating:⚡ AND keyword IN ["diffusion", "flow matching"] THEN flag_for_ai_app_review` | LLM 自由发现跨域关联假阳性率过高（"两个领域都提到了 transformer"）。确定性规则可审计、可预测，且编码了用户自身的跨域假设，而非让模型去猜测 | ✅ 已完成（v2：7 条规则 + LLM 意义说明） |
| **P1** | [**场态触发器**](docs/use-cases/field-state-trigger.md) | 零 LLM 闸门，决定当天 deep-dive 是否应该跑；6 类触发器（突破密度、范式偏移、共识漂移、静默衰减、跨域拉力、发布扎堆） | 每天都生成 deep-dive 的 pipeline，迟早会在淡日产出**噪音 deep-dive**。场态触发器让系统在没东西可说时安静下来，并给出可审计的理由；既钉死 LLM 成本又不漏掉时效信号 | ✅ 已完成 |
| **P2** | [**GitHub Issues 采纳度传感器**](docs/use-cases/gh-issues-adoption-sensor.md) | 监控 OSS 仓库 issue/PR 节奏（而非 star 数），推断采纳阶段（孵化→成长→主流→成熟）、Daily Field Index、跨仓库收敛信号 | Star 衡量关注度，不衡量采纳度。"这个库正在赢吗"的维护者视角信号藏在 issue 节奏和贡献者多样性里；4 脚本管线把这变成可 grep 的结构化场态图 | ✅ 已完成 |
