...
系統不只在跑，它在學習自己哪些判斷需要更多佐證，然後主動去找。
...
| 維度 | 一般監控 / 聚合工具 | Pulsar |
|------|-------------------|--------|
| 故障恢復 | 手動重啟 | Watchdog 自動恢復 7 類故障 |
| 知識沉澱 | 消息流 / 收件箱 | 結構化 Markdown，Git 歷史，可 grep |
| 信念更新 | 靜態假設 | 19 條假設，置信度月度自動調整 |
| 預測問責 | 無追蹤 | ✅/❌ 每兩週回顧，正確率有歷史記錄 |
| 信號優先級 | 僅按時間倒序 | ⚡/🔧/📖/❌ 先評分再進 LLM，節省成本 |
| 資源佔用 | 雲端規模 | 2 GB RAM 即可，個人服務器可用 |
| 領域深度 | 廣泛 / 淺層 | 兩個深耕領域：VLA 機器人 + AI 應用生態 |
