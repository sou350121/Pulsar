...
| **多域配置** | `memory/domains.json` 注册表 + `scripts/_domain_loader.py` 共享加载器——新增第三个研究域只需修改一个文件，无需改动现有脚本 |
...
| **P0** | **多域配置** | 在同一调度与交付层下扩展至 N 个域，每个域拥有独立的配置文件和内存路径 | 所有跨域功能的结构性前提；配置层已部分支持，需要 pipeline 统一化和路由逻辑 | ✅ Done |
| **P0** | **一键部署脚本** | 交互式 `setup.sh`，在单次引导运行中完成 `.env`、`active-config.json`、GitHub 配置和首次 cron 加载的全部搭建 | 将复制部署的摩擦从约 1 小时降至数分钟；社区采用率取决于此；第一印象决定是否有人真正克隆并运行 | ✅ Done |
